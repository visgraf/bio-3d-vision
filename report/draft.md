# Technical report — drafted prose

**Source: Chat surface. NOT derived from this repository.**

This file is the input to typesetting, not a deliverable. Every factual claim in
it has been checked against `report/claims-verified.md`, and the corrections and
qualifications recorded there are already applied. Where this file and
`claims-verified.md` disagree, **`claims-verified.md` wins** — report the
disagreement rather than resolving it silently.

Figure placements are marked `[FIGURE n]` with a description. Only Figure 1
exists (`report/figures/scanpath.pdf`); the rest are to be generated.

---

# 1. Introduction

## 1.1 Motivation: active vision as sampling under a budget
...
It also made a defect in that fixture unusually damaging, in a way §2 takes up.

## 1.2 Background: stereo depth as inference
...
A fifth concept was not brought to the project but forced on it, and §2 begins
there: a stimulus is an instrument, and must be characterised before it is used
to measure.

## 1.4 Architecture

[FIGURE 2 — the six-layer architecture, with what was built, what was measured,
...
branch left untested; L6 is a two-line argmax that, as §2.2 reports, turned out
not to matter.
...
§2.1 asks what limits availability, §2.2 what selection buys, §2.3 what changes
availability.
...
[FIGURE 1 — the scanpath under argmax-variance without inhibition of return.
...
recovers, and it is better described as badly steered than as stuck. §3 returns
...
[FIGURE 4 — coverage against accuracy for the two arms: what each saw, and how
...
[FIGURE 5 — spherical stereo geometry: epipolar great circles as meridians about
...
up the question §3 takes as its subject.
...
## 3.3 On simulation as an instrument
...
**Variable-resolution sampling** is the priority, for the reason §3.2 gives — it
...
[FIGURE 6 — the foreclosure ledger as a timeline, showing which conclusions were
